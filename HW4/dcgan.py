...
for epoch in range(epochs):
    for i, (real_images, _) in enumerate(dataloader):
        ...
        if i % 100 == 0:
            print(f'[{epoch+1}/{epochs}][{i}/{len(dataloader)}] Loss_D: {errD.item():.4f} Loss_G: {errG.item():.4f} D(x): {D_x:.4f} D(G(z)): {D_G_z1}/{D_G_z2}')
    ...
    save_image(fake_images, './figure/dcgan/fake/epoch_%03d.png' % (epoch), nrow=8, normalize=True)
    save_image(real_images, './figure/dcgan/real/epoch_%03d.png' % (epoch), nrow=8, normalize=True)


plt.figure(figsize=(10,5))
plt.title("DCGAN Loss During Training: Generator and Discriminator")
...
plt.plot(np.arange(0,50), fid_scores, label = 'DCGAN')
...
g_losses_path = './result/dcgan_g_losses.txt' 
d_losses_path = './result/dcgan_d_losses.txt'
fid_scores_path = './result/dcgan_fid_scores.txt'
